...
router.get('/accounts', needUserUUID,async(req:Request, res:Response, next:NextFunction)=>{
  try {
    fs.readFile(`./db/keystores/${req.uuid}.json`,'utf8',async(err, data)=>{
      ...
      const keystore=await lightwallet.keystore.deserialize(d.keystore);
  ...
  const password=req.body.password;
      ...
      try {
          fs.readFile(`./db/keystores/${req.uuid}.json`,'utf8',async(err, data)=>
          {
            const d:UserWalletInfoT = JSON.parse(data);
              const keystore=await lightwallet.keystore.deserialize(d.keystore);
              await keystore.keyFromPassword(password,(err,pwDerivedKey)=>{
                   ...
                   fs.writeFile(`./db/keystores/${req.uuid}.json`, JSON.stringify(saving), (err)=> console.log(err));
...
/**
   ...
   *         
   */
router.post('/privatekey/:address', needUserUUID, async(req:Request, res:Response, next:NextFunction)=>{
  const address=req.params['address'];
  const password=req.body.password;
  try {
      fs.readFile(`./db/keystores/${req.uuid}.json`,'utf8',async(err, data)=>{
